...
const YourOrders = () => {
  ...
  const completeOrder = async (id) => {
    ...
    const contract = new ethers.Contract(
      "0xB5CACb6aC2A321710AeececC0Fb40625b404F646",
      abi,
      signer
    );
    const payment = ethers.parseEther("0.0085");
    await contract.completeOrder(
      "0x3CF054dC8C404550d69af465c2eBcC1136a8AE6f",
      payment
    );
    a.push(id);
    console.log("A", a);
  ...
  return (
    <div className="all-orders p-5 h-[70vh]">
      {OrdersData.map((item) => {
        return (
          <div className="w-[100%]  border-2 shadow-md rounded-lg  overflow-hidden flex	">
            <div className="w-[30%]">
            ...
            <div className="p-3">
              ...
              <div className="button flex justify-end p-2">
                <button
                  className="bg-sky-600 text[12px] px-3 p-[6px] text-white rounded-lg"
                  onClick={() => {
                    data.includes(item.title)
                      ? console.log(item.title)
                      : completeOrder(item.title);
                  }}
                >
                  {data.includes(item.title) ? "Completed" : "Complete Order"}
                </button>
              </div>
            </div>
          </div>
...
export default YourOrders
>>>>>>> 1358cd808c01025940b6766775bbc0b5b1909ff4
